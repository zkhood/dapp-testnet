...
const ENCLAVE = process.env.ENCLAVE || "http://65.1.36.224:4000";
const NITRO = process.env.NITRO || "http://65.1.36.224:1300";
...
// Operator server path first, falling back to the copy bundled alongside this script (a plain
// public address list — safe to ship — so this CLI also runs standalone off the server layout).
const DEPLOY_PATH = process.env.DEPLOYMENT_JSON
  || (fs.existsSync("/root/zkhood/packages/contracts/deployments/robinhoodTestnet.json")
    ? "/root/zkhood/packages/contracts/deployments/robinhoodTestnet.json"
    : path.join(__dirname, "deployments", "robinhoodTestnet.json"));
...
const RELAYER = ethers.getAddress("0xb18CD02AceC52Fae908F28df8DFd4e612522b00A");
